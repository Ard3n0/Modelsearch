...
for name, model in models.items():
    ...
    recall = recall_score(y_true_k, y_pred_k, zero_division=0)
    map_score = average_precision_score(y_test, probs)
    results_table['Model'].append(name)
    results_table['Stage'].append('Baseline (3.3)')
    results_table['Precision@10'].append(precision)
    results_table['Recall@10'].append(recall)
    results_table['mAP'].append(map_score)
...
precision = precision_score(y_true_k, y_pred_k, zero_division=0)
recall = recall_score(y_true_k, y_pred_k, zero_division=0)
map_score = average_precision_score(y_test, stacked_probs)
...
model_stages = [f"{row['Model']} ({row['Stage']})" for _, row in df.iterrows()]
x = np.arange(len(model_stages))
width = 0.25
...
ax.set_xticklabels(model_stages, rotation=45, ha='right')
ax.set_ylabel('Значение метрики')
ax.set_title('Сравнение метрик моделей')
ax.legend()
plt.tight_layout()
plt.savefig('metrics_comparison_improved.png')
plt.close()
